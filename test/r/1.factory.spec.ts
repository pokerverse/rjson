...
describe("r ProjectFactory tests", () => {
  it("should add a media upload element both at root scene level and inside group", () => {
    ...
    const groupF = r.record(group as R.RecordNode<R.RT.element>);
    const groupElements = groupF.getRecords(RT.element);
    const groupElementTypes = groupElements.map(el => el.props.element_type);
    expect(groupElementTypes.includes("media_upload"));

    const sceneElements = sceneF.getRecords(RT.element);
    const sceneElementTypes = sceneElements.map(el => el.props.element_type);
    expect(sceneElementTypes.includes("media_upload"));
  });
  ...
  it("should duplicate scorm element with new linked variables attached", () => {
    ...
    if (embedScormElementTwo) {
      ...
      expect(scormElements[0].props.embed_scorm_score_var_id as number).to.not.equal(embedScormElementTwo.props.embed_scorm_score_var_id as number);
      expect(scormElements[0].props.embed_scorm_suspend_data_var_id as number).to.not.equal(embedScormElementTwo.props.embed_scorm_suspend_data_var_id as number);
      expect(scormElements[0].props.embed_scorm_progress_var_id as number).to.not.equal(embedScormElementTwo.props.embed_scorm_progress_var_id as number);
    }
  });
});
